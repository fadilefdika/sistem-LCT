@forelse($laporans as $index => $laporan)
                        ...
                        <tr @click="openRow === {{ $laporanId }} ? openRow = null : openRow = {{ $laporanId }}" class="hover:bg-gray-100 cursor-pointer text-sm transition duration-200 ease-in-out">
                            ...
                            @php
                                ...
                                <td class="px-4 py-3 w-32 text-[11px] whitespace-nowrap {{ $isOverdue ? 'text-red-600 font-semibold' : 'text-gray-800' }}">
                            ...
                            <!-- Completion Date -->
                            <td class="px-4 py-3 text-gray-800 w-32 whitespace-nowrap text-[11px]">
                                {{ $laporan->date_completion ? \Carbon\Carbon::parse($laporan->date_completion)->format('F d, Y') : '-' }}
                            </td>
                            <td class="px-4 py-3 w-36 text-gray-800 whitespace-nowrap text-[11px]">{{ $laporan->area->nama_area }} - {{ $laporan->detail_area }}</td>
                            <!-- Tingkat Bahaya -->
                                                    ...
                                                    <div class="lg:w-1/3 w-full" x-data="{
                                                        ...
                                                        <div class="relative aspect-video bg-gray-100 rounded-md overflow-hidden border border-gray-200">
                                                            ...
                                                            <template x-if="images.length > 1">
                                                                <button @click="prev"
                                                                    class="absolute cursor-pointer left-0 top-1/2 -translate-y-1/2 bg-white/60 hover:bg-white text-gray-800 px-2 py-1 cursor-pointer rounded-r-md">
                                                                    ‹
                                                                </button>
                                                            </template>
                                                            ...
                                                            <template x-if="images.length > 1">
                                                                <button @click="next"
                                                                    class="absolute cursor-pointer right-0 top-1/2 -translate-y-1/2 bg-white/60 hover:bg-white text-gray-800 px-2 py-1 cursor-pointer rounded-l-md">
                                                                    ›
                                                                </button>
                                                            </template>
                                                        </div>
