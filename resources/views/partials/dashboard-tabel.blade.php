<div class="overflow-x-auto rounded-lg border border-gray-200">
    <table class="min-w-full divide-y divide-gray-300 shadow-sm border border-gray-200 rounded-lg overflow-hidden">
        ...
        <tbody class="divide-y divide-gray-200 bg-white">
            ...
            <tr class="hover:bg-gray-100 text-xs transition duration-200 ease-in-out">
                <td class="px-3 py-2 text-center font-semibold text-gray-800 w-10">
                    {{ $loop->iteration }}
                </td>
                <td class="px-3 py-2 text-gray-800 max-w-xs truncate cursor-pointer relative group">
                    <span class="temuan-clamp" title="{{ $laporan->temuan_ketidaksesuaian }}">
                        {{ $laporan->temuan_ketidaksesuaian }}
                    </span>
                </td>
                <td class="px-3 py-2 text-gray-800 w-32">{{ $laporan->picUser->fullname ?? '-' }}</td>

                <!-- Tingkat Bahaya -->
                <td class="px-3 py-2 text-gray-800 w-24">
                    @php
                        $bahayaColors = [
                            'High' => 'bg-red-500',
                    ...
                    @endphp
                    <span class="px-2 py-1 text-xs font-semibold text-white rounded-full {{ $bahayaColors[$laporan->tingkat_bahaya] ?? 'bg-gray-400' }}">
                        {{ $laporan->tingkat_bahaya }}
                    </span>
                </td>

                <!-- Status Progress -->
                <td class="px-3 py-2 text-gray-800 w-28">
                    @php
                        $statusMapping = [
                    ...
                    <span class="inline-flex items-center justify-center px-2 py-1 text-[8px] font-semibold text-white rounded-full {{ $status['color'] }} whitespace-nowrap">
                        {{ $status['label'] }}
                    </span>
                </td>
                ...
                <td class="px-3 py-2 text-[10px]">
                    <span class="inline-flex items-center justify-center px-2 py-1 text-black rounded-full whitespace-nowrap">
                        {{ $status['tracking'] }}
                    </span>
                </td>

                <!-- Tenggat Waktu -->
                <td class="px-3 py-2 text-gray-800 w-28 whitespace-nowrap">
                ...
                <td class="px-3 py-2 text-gray-800 w-28 whitespace-nowrap">
                    ...
                    @else
                        ...
                        @elseif ($overdueDays === 0)
                            <span class="bg-yellow-100 text-yellow-600 text-xs font-semibold px-2 py-1 rounded">
                                Due Today
                            </span>
                        @else
