<div class="rounded-lg">
    ...
    @if (($laporan->status_lct === 'revision' && $laporan->tingkat_bahaya == "Low")|| ($laporan->status_lct === 'temporary_revision' && in_array($laporan->tingkat_bahaya, ['Medium','High'])) || $laporan->tindakan_perbaikan)
                ...
                @if (!empty($tindakan_perbaikan[0]['bukti']))
                    ...
                    <div class="flex overflow-x-auto gap-2">
                        @foreach ($tindakan_perbaikan[0]['bukti'] as $img)
                            <img src="{{ $img }}" class="w-24 h-24 object-cover rounded-lg cursor-pointer hover:scale-110 transition" loading="lazy" onclick="openModal('{{ $img }}')" alt="Proof">
                        @endforeach
                    </div>
                @endif
            </div>
        ...
        @if (($laporan->status_lct === 'revision' && $lowOrTemporaryRejects->isNotEmpty())||($laporan->tindakan_perbaikan && $lowOrTemporaryRejects->isNotEmpty()))
            @if ($hasRevisions)
            {{-- @php 
                dd($lowOrTemporaryRejects->pluck('tipe_reject'));
            @endphp     --}}
